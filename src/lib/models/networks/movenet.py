...
joints_name = ["nose", "left eye", "right eye", "left ear", "right ear",
...
class MoveNet(nn.Module):
    ...
    def decode(self, x):
        ...
        kpt_coor = self._center_to_kpt(kpt_regress, ct_ind, ft_size= self.ft_size)

        # kpt_coor = kpt_coor / self.ft_size
        # score = torch.ones([17, 1])
        # kpt_with_conf = torch.cat([kpt_coor, score], dim = 1).reshape((1, 1, 17, 3))
        # kpt_with_confs.append(kpt_with_conf)
    ...
    def _center_to_kpt(self, kpt_regress, ct_ind, ft_size=48):
        ...
        print(f"_center_to_kpt ct_x {ct_x}, ct_y {ct_y} {ct_ind} {ft_size}")
    ...
    def _kpt_from_heatmap(self, kpt_heatmap, kpt_coor):
        print(f'_kpt_from_heatmap {kpt_heatmap.shape}')
        # _kpt_from_heatmap torch.Size([64, 64, 17])
        plt.figure()
        for i in range(17):
            plt.subplot(6,3,i + 1)
            plt.title(joints_name[i])
            plt.imshow(kpt_heatmap[:,:, i])
        plt.show()
...
def get_pose_net(heads, head_conv=96, froze_backbone=True, model_type = 'lighting'):
    ...
    for k,v in model.named_parameters():
        head_name = k.split('.')[0]
        print(f'get_pose_net {k} {head_name} {v.requires_grad}')
        if head_name == 'wh' or head_name == 'reg':
