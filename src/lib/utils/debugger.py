...
class Debugger(object):
    ...
    def draw_skel_and_kp(
        ...
        print(f'draw_skel_and_kp kpt_with_conf {kpt_with_conf.shape}')

        out_img = img
        height, width, _ = img.shape
        adjacent_keypoints = []
        cv_keypoints = []
        keypoint_scores = kpt_with_conf[:, 2] if kpt_with_conf.ndim == 3 else np.ones(kpt_with_conf.shape[0])
    ...
    def add_coco_hp(self, kpt_with_conf, img_id='default', vis_thresh=0.3, reshape=3):
        self.imgs[img_id] = self.draw_skel_and_kp(self.imgs[img_id], kpt_with_conf.reshape(-1, reshape), vis_thresh)
