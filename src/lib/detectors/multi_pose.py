...
from utils.post_process import multi_pose_post_process, uni_multi_pose_post_process
...
class MultiPoseDetector(BaseDetector):
  ...
  def post_process(self, dets, meta, scale=1):
    ...
    dets = uni_multi_pose_post_process(
      dets.copy(),
  ...
  def show_results(self, debugger, image, results, multi_pose = False):
    print(f'show_results {results.shape}')
    debugger.add_img(image, img_id='multi_pose')
    for bbox in results:
      if bbox[4] > self.opt.vis_thresh:
        debugger.add_coco_bbox(bbox[:4], 0, bbox[4], img_id='multi_pose')
        debugger.add_coco_hp(bbox[5:39], img_id='multi_pose', reshape=2)
